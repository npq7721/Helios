 ...
 * between heliosd, and helios-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    FlushWallets();
#endif
    GenerateHelioss(false, 0, Params());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), helios_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/HeliosProject/Helioscoin>";
    const std::string URL_WEBSITE = "<https://helioscoin.org>";
...
/** Sanity checks
 *  Ensure that Helios is running in a usable environment with all
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Helios process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", helios_CONF_FILENAME)).string());
    ...
    GenerateHelioss(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
