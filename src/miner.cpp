...
void static HeliosMiner(const CChainParams& chainparams)
{
    LogPrintf("HeliosMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("helios-miner");
    ...
    if (!EnsureWalletIsAvailable(pWallet, false)) {
        LogPrintf("HeliosMiner -- Wallet not available\n");
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("HeliosMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
    ...
    {
        LogPrintf("HeliosMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("HeliosMiner -- runtime error: %s\n", e.what());
        return;
    }
}

int GenerateHelioss(bool fGenerate, int nThreads, const CChainParams& chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++){
        minerThreads->create_thread(boost::bind(&HeliosMiner, boost::cref(chainparams)));
    }
