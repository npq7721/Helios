...
The RPM spec file provided here is for Helios-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://helios.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Helios git repository](https://github.com/HeliosProject/Helioscoin) but are not part of
...
Currently only `Source10` is used, for the example `helios.conf` file.
...
in the helios git repository. At present only an SVG version of the Helios
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the helios
...
needed when 0.12.1 is released, a proper fix is already in the Helios git
master branch.
...
idea to build Helios Core against a static version of that library compiled
...
The `helios.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `helios-server` package. At this point, most RPM based
...
The `helios-server` package should have SELinux support. How to properly do
...
Helios-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/helios/). He has been
packaging Helios for Fedora at least since 2012.
...
4. I renamed the `helios` package that contains the Qt GUI to `helios-core` as
that appears to be how the general population refers to it, in contrast to
`helios-xt` or `helios-classic`. I wanted to make sure the general population
