...
###   helioscore.org and helios.org and compares them.
...
WORKINGDIR="/tmp/helios_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://helioscoin.org"
HOST2="https://helios.org"
BASEDIR="/bin/"
VERSIONPREFIX="helios-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'helios-',
...
if [ $? -ne 0 ]; then
   echo "helios.org failed to provide signature file, but helioscore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "helios.org and helioscore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Helios Core binary release signing key installed?"
   fi
